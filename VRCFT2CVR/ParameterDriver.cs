...
{
    #region ManuallyPullingVRCFTParameters (Hypernex.Unity)
    /*
     ...
     * Changes:
     *   + UpdateParameters was changed to private
     *   + UpdateParameters parameter types were changed to List<CVRAnimatorManager.ParamDef>, ParameterVersion
     *   + UpdateParameters sets the id and name of the avatarConfigSpec to CVRAvatarId and CVRAvatar respectively
     *   + UpdateParameters pulls parameters from the paramDef parameter
     *   + ICustomFaceExpression was set to private
     */
    
    public static bool UseBinary { get; set; } = true;
    
    private static List<ICustomFaceExpression> CustomFaceExpressions = new();

    private static void UpdateParameters(List<CVRAnimatorManager.ParamDef> paramDef, ParameterVersion parameterVersion = ParameterVersion.Both)
    {
        List<AvatarConfigFileParameter> parameters = new List<AvatarConfigFileParameter>();
        paramDef.Where(x => x.type != AnimatorControllerParameterType.Trigger).ToList().ForEach(
                parameter =>
                {
        ...
        CustomFaceExpressions.RemoveAll(x => x is VRCFTProgrammableExpression);
        ...
        GetParameters(paramList.ToArray()).ForEach(parameter => CustomFaceExpressions.Add(parameter));
    }
    ...
    private class VRCFTProgrammableExpression : ICustomFaceExpression
    {
        ...
        private Regex Regex
        {
            get
            {
                ...
                {
                    lastRegexName = Name;
                    lastRegex = VRCFTTools.GetParameterRegex(lastRegexName);
                    matches.Clear();
                }
                return lastRegex;
            }
    ...
    private interface ICustomFaceExpression
    {
        string Name { get; }
        float GetWeight(UnifiedTrackingData data);
    ...
    public void Update(UnifiedTrackingData data)
    {
        ...
        {
            CustomFaceExpressions.ForEach(expression =>
            {
